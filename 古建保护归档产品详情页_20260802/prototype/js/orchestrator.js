...
window.Orchestrator = (function () {
  ...
  async function start(userText) {
    ...
    if (!API.isReady()) {
      Store.say("ai", "在线识别服务尚未连接，暂时不能读取这项新任务。" +
        "你可以先用高都玉皇庙的示例资料体验完整流程，页面会清楚标出哪些内容是示例。", {
        card: { key: "no-key", title: "接下来怎么走",
          options: [
            { label: "体验示例流程", sub: "使用高都玉皇庙的示例资料", value: "demo" },
    ...
    const 是本机项目 = 匹配预判 === "gaodu";
    const 有数据 = 是本机项目;

    /* 新项目必须从空任务卡开始。
       否则用户建的是上海的项目，任务卡里却留着高都玉皇庙的规范和交付格式，
    ...
    if (!有数据) {
      ...
      Store.say("ai",
        ...
        "目前只有高都玉皇庙主殿配有完整的照片、任务书和实测记录。" +
        "我可以把你刚才说的要求记成任务卡，但接下来的识别、出图只能用高都玉皇庙的数据演示。", {
        card: { key: "no-project", title: "接下来怎么走",
          options: [
            { label: "用高都玉皇庙的数据继续演示", sub: "任务卡保留你说的要求", value: "demo" },
  ...
  async function demoIntake() {
    Store.goto("task");
    await think("正在读取演示资料", "使用预置的高都玉皇庙数据", 800);
    Store.say("ai", "（示例资料）任务要求为正立面现状测绘图，比例 1:50，" +
      "参照北京《历史建筑数字化技术规范》征求意见稿，交付 DWG 和 PDF。以上内容是页面预置示例，不是本次识别结果。");
    await UI.sleep(300);
  ...
  async function readBrief() {
    ...
    try {
      const r = await API.jsonTask(CFG.briefPrompt(DATA.任务书原文), { step: "读任务书" });
      const 卡 = S().任务卡;
  ...
  async function afterDatum() {
    ...
    if (API.isReady()) {
      try {
        const 清单 = S().资料.map(x =>
          "- [" + x.类型 + "] " + x.名称 + "：" + (x.说明 || "") +
          (x.可用 ? "" : "（不存在）")).join("\n");
        const r = await API.jsonTask(CFG.materialsPrompt(清单), { step: "资料判断" });
        S().资料判断 = r;
        m占位.text = (r.说明 || "") +
          (r.缺口 && r.缺口.length
      ...
      m占位.text = "（示例资料）4 张照片中有 1 张正立面全景可作主图，右次间下部被树木遮挡。";
    ...
    if (API.isReady()) {
      try {
        const r = await API.jsonTask(CFG.sketchPrompt(DATA.草图转写), { step: "草图解析" });
        applySketch(r);
        d占位.text = (r.说明 || "") + "\n\n" +
      ...
      d占位.text = "（示例资料）草图中共有 8 个尺寸，5 个能对应部位，3 个没有标注部位。";
    ...
    if (chk.length) {
      ...
      Store.say("ai",
        ...
        "超出任务书 ±20mm 的精度要求。可能是通面阔含副阶或廊步，也可能某项实测有误。" +
        "需要现场复核后才能建立比例基准。这项结果按固定公式计算，可以重复验证。",
    ...
    stopAt("等你确认", "3 个尺寸部位不明，另有 1 处尺寸矛盾");
  }
  ...
  async function runRecognize(note) {
    ...
    if (真实) {
      const vm = API.model("vision");
      const 预计 = /^kimi-k2/.test(vm) ? "一分多钟" : "二十多秒";
      Store.say("ai", "正在重新识别构件，预计需要" + 预计 + "。" +
        (note ? "我会带着你的疑问重点核对：" + note : ""));
      await think("正在识别构件", "读取主图并定位构件", 300);
      try {
        const url = await API.urlToDataUrl("assets/gaodu_main.jpg", 1024);
        const r = await API.recognize(url, visionCtx("山西晋城高都玉皇庙主殿，三开间，前廊敞开" +
    ...
    Store.say("ai",
      ...
      "每个都标了在照片上的位置和尺寸预估。材料主要是木、青砖、石和布瓦。\n\n" +
      "有 " + S0.存疑.filter(q => !q.已解决).length + " 项证据不足，已经排在清单最上面，需要你判断。" +
      "另外提醒：这座殿前廊是完全敞开的，没有槛墙也没有门窗，和常见的三开间殿不一样，图上不要按惯例补门窗。");
  ...
  async function afterParts() {
    ...
    Store.say("ai",
      "现状记录整理好了。开间三间有实测支持，前廊敞开、方形石柱、灰布瓦这几项是照片外观判断，" +
      "材质和病害没有检测依据，按规范要标注为外观判断。右次间下部遮挡处标为不可见部位。");
  ...
  async function runDrawing() {
    ...
    if (pend.length && API.isReady()) {
      ...
      try {
        ...
        const r = await API.jsonTask(CFG.checkExplainPrompt(摘要), { step: "检查解释" });
  ...
  async function afterDrawing() {
    ...
    if (API.isReady()) {
      try {
        const r = await API.jsonTask(CFG.deliveryPrompt(buildContext()), { step: "交付说明" });
  ...
  async function onChoice(key, value, label) {
    ...
    if (key === "datum-conflict") {
      if (value === "keep") {
        Store.log("处理尺寸矛盾", "通面阔", "按通面阔建立基准，差值 4400mm 标注待复核");
        Store.say("ai", "好。基准按通面阔 15800 建立，差值 4400 已记入待复核清单，会写进交付说明的限制条件。");
      } else {
  ...
  async function proposeEdits(instruction) {
    if (!API.isReady()) {
      Store.say("ai", "在线识别服务尚未连接，暂时不能生成修改建议。请先在“服务与偏好”中检查连接。");
      return;
    }
  ...
  function applySketch(r) {
    ...
    (r.已识别 || []).forEach(x => {
      ...
      if (hit) { hit.部位 = 位; hit.状态 = "measured"; hit.说明 = x.依据 || "从草图中识别"; }
      else 表.push({ id: "d" + (表.length + 1), 部位: 位, 数值: Number(x.数值),
        单位: "mm", 方式: "钢尺", 状态: "measured", 说明: x.依据 || "从草图中识别" });
    });
    (r.无法确定 || []).forEach(x => {
  ...
  function buildContext() {
    const s = S();
    const 状态名 = { measured: "现场实测", ai: "AI 识别", human: "人工确认", unknown: "待确认" };
    const L = [];
    ...
    L.push("成果：" + DATA.项目.成果 + "，比例 1:50，规范：北京《历史建筑数字化技术规范》征求意见稿");
    L.push("精度要求：主要控制尺寸误差不超过 ±20mm");
    L.push("当前所在步骤：" + ((DATA.步骤.find(x => x.视图 === s.当前视图) || {}).名 || s.当前视图));

    L.push("");
    L.push("== 实测记录（现场实测，是唯一的尺寸基准）==");
    s.实测.forEach(d => {
    ...
    L.push("");
    L.push("== 已知限制 ==");
    L.push("- 右次间下部被树木遮挡，两张照片同一位置均无法确认");
    L.push("- 材料信息为外观判断，没有取样检测依据");
    L.push("- 这座殿前廊完全敞开，无槛墙无门窗，与常见三开间殿不同，不要按惯例补门窗");
    L.push("- 标注为 AI 识别的尺寸来自照片比例估算，不是实测值");
  ...
  function refAdd(名称, ref, text) {
    ...
    pendingSym = { 名称, 类别: guessCat(名称), 参照: p.编号, 尺寸: p.尺寸 || "" };
    let 回 = "找到了，位置在你框的这块。我按「" + 名称 + "」补录了 " + p.编号 + "。";
    if (对称) 回 += "右边有一个对称的 " + 对称.编号 + "，尺寸我按对称关系推算，标为推测值等你确认。";
    回 += "\n\n顺便问一下，右次间下部被树挡住的那几处，是不是也是同样的做法？如果是我可以先按同样方式补上，都标成推测。";
  ...
  async function handleRefEdit(text, ref) {
    ...
    if (API.isReady()) {
      ...
      try {
        const r = await API.jsonTask(CFG.refEditPrompt(text, ref.命中), { step: "框选理解" });
        const a = r.动作 || "";
  ...
  async function onDrop(file) {
    ...
    let 分类;
    try {
      分类 = await API.visionTask(url, CFG.classifyPrompt(), { step: "资料分类" });
    } catch (e) {
  ...
  async function 处理草图(url, 名) {
    ...
    const r = await API.visionTask(url, CFG.sketchImagePrompt(), { step: "草图识别" });
  ...
  async function 处理任务书(url, 名) {
    ...
    const r = await API.visionTask(url, CFG.briefImagePrompt(), { step: "任务书识别" });
  ...
  function onChoiceRouter(key, value, label) {
    ...
    if (key === "no-project") {
      ...
      Store.say("ai", "将使用高都玉皇庙的资料继续。你刚才说明的要求会保留，并标为“AI 识别”；其余内容标为“示例资料”。");
      return readBrief();
    }
    if (key === "offer-apply") {
    ...
    if (key === "sym") {
      if (value && pendingSym) {
        const 区 = DATA.遮挡区域 || { 部位: "右次间下部", 框: [0.63, 0.42, 0.855, 0.66] };
        const p = Store.addPart({
