...
window.PROV = (function () {
  ...
  const DEF = {
    ...
    rule:     { 名: "自动核对", 说明: "按固定检查规则得出，可重复验证", cls: "prov-rule" },
  ...
  function name(k) { return (DEF[k] || DEF.demo).名; }
  function desc(k) { return (DEF[k] || DEF.demo).说明; }

  /* 统计当前工作区里各类来源各占多少，用于向用户交代 AI 到底参与了多少 */
  function summarize(S) {
    const c = { demo: 0, ai: 0, rule: 0, measured: 0, human: 0, unknown: 0, missing: 0 };
    const bump = k => { if (c[k] != null) c[k]++; else c.demo++; };
